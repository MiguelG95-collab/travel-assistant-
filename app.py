...
st.set_page_config(page_title="Travel Assistant", page_icon="✈️", layout="wide", initial_sidebar_state="expanded")
...
st.markdown("""
<style>
:root{--bg:#f6f7f9;--surface:#fff;--text:#17181a;--muted:#6f737a;--border:#e6e8eb;--accent:#111827}
...
h1{font-size:2rem!important;font-weight:750!important;margin-bottom:.25rem!important}
h2{font-size:1.35rem!important;font-weight:700!important}
p,.stCaption{color:var(--muted)}
[data-testid="stMetric"]{background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:16px 18px;box-shadow:0 1px 2px rgba(0,0,0,.025)}
[data-testid="stMetricLabel"]{font-size:.78rem;text-transform:uppercase;letter-spacing:.05em;font-weight:700;color:var(--muted)}
[data-testid="stMetricValue"]{font-size:1.65rem;font-weight:750;color:var(--text)}
div[data-testid="stForm"]{background:#fff;border:1px solid var(--border);border-radius:16px;padding:20px}
div[data-baseweb="tab-list"]{gap:6px;background:#eceff3;padding:5px;border-radius:12px}
button[data-baseweb="tab"]{border-radius:9px;padding-left:16px;padding-right:16px}
.stButton>button,.stDownloadButton>button{border-radius:10px;font-weight:650;min-height:42px}
div[data-baseweb="select"]>div,.stTextInput input,.stTextArea textarea,.stNumberInput input,.stDateInput input{border-radius:10px!important}
[data-testid="stDataFrame"]{border:1px solid var(--border);border-radius:14px;overflow:hidden;background:#fff}
[data-testid="stExpander"]{background:#fff;border:1px solid var(--border);border-radius:14px}
hr{border-color:var(--border)}
.card{background:#fff;border:1px solid var(--border);padding:18px;border-radius:16px;box-shadow:0 1px 2px rgba(0,0,0,.02);margin-bottom:12px}
.app-brand{font-size:1.22rem;font-weight:800;letter-spacing:-.03em;color:#111827;margin-bottom:.1rem}
.app-tagline{font-size:.8rem;color:#858a92;margin-bottom:1rem}
</style>
...
with st.sidebar:
    st.markdown('<div class="app-brand">Travel Assistant</div><div class="app-tagline">Sistema para agencia de viajes</div>', unsafe_allow_html=True)
    page = st.radio(
        "Menú",
    ...
    st.markdown("---")
    st.caption("Versión profesional gratuita · OpenStreetMap + Open-Meteo")
